...
\end{flushright}
    ...
    \par
    Fig.\ref{fig:blade xy}に$xy$平面から見たブレードを示す．$dF$は，ブレードの微小要素にはたらく遠心力であり，Eq.\eqref{centri1}で表される．
    ...
    Fig.\ref{fig:blade yz}に$yz$平面から見たブレードを示す．ブレードの微小要素にはたらく遠心力がつくるねじり下げモーメントは
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[keepaspectratio, width=0.4\linewidth]{photo/blade_xy_axis.jpg}
        \caption{Blade viewed from $xy$ plane}
        \label{fig:blade xy}
    \end{figure}
    \begin{figure}[H]
        \centering
        \includegraphics[keepaspectratio, width=0.6\linewidth]{photo/blade_yz_axis.jpg}
        \caption{Blade viewed from $yz$ plane}
        \label{fig:blade yz}
    \end{figure}
    ...
    \par 
    Table.\ref{table:rotor type}にロータのピッチタイプを示す．ロータの種類については固定ピッチロータと可変ピッチロータがある．
    ...
    \begin{table}[H]
        \centering
        \caption{Rotor type}
        \label{table:rotor type}
        \setlength{\tabcolsep}{5pt}
        \begin{tabular}{|c|c|c|} \hline
            Rotor type & Fixed pitch rotor & Variable pitch rotor \\ \hline
            merit & 
            \begin{tabular}[c]{@{}c@{}}
                low cost \\
                light weight \\
    ...
    \section{ブレード解析モデル}
    \par
    Fig.\ref{fig:blade model}に解析対象となるブレードモデルを示す．ブレードの直径は$0.686\ \mathrm{m}$，平面形はテーパ比0.603のテーパ翼で統一した．
    ブレードの翼型はOAF117で検討する．ブレードの表皮はすべて$3\ \mathrm{mm}$であるとする．Table.\ref{table:blade spec}のようなブレード諸元について
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[keepaspectratio, width=0.8\linewidth]{photo/blade_calc_model.jpg}
        \caption{Blade model}
        \label{fig:blade model}
    \end{figure}

    \begin{table}[H]
        \centering
        \caption{Blade specification}
        \label{table:blade spec}
        \setlength{\tabcolsep}{5pt}
        \begin{tabular}{cccccccc} \hline
            ...
            NB2  & OAF117  &       2          &    -21     &  0.0362    &  0.06      &    5.133     & 0.0642   \\    
            NB4  & OAF117  &       4          &    -21     &  0.0362    &  0.06      &    5.133     & 0.1286   \\    \hline
        \end{tabular}
    \end{table}
