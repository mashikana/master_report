...
\end{flushright}
    ...
    代表的な構成として,単一のメインロータと機体尾部のテールロータによってトルクを制御するシングルロータヘリコプタ（1）が挙げられる.
    一例として,SH-60K哨戒ヘリコプタが挙げられる.
    さらに,前後方向に配置された2つのロータが互いのトルクを打ち消すことで機体を安定化させるタンデムロータヘリコプタ（2）が知られている.
    この形式には,CH-47Jのような機体がある.
    タンデムロータ機は，ロータが機体前後方向に存在するため，胴体の重心移動に対してロバスト性を有する特徴を持つ．
    また,二重反転式ロータヘリコプタ（３）として,Ka-32等の上下二段で反対方向に回転するロータを備えた形式が存在し,テールロータを不要としている.
    さらに,二重反転ロータの機構を簡素化し整備性を向上させた交差反転式ロータヘリコプタ（４）として,K-MAX のような機体も存在する.（図1-1）
    ...
    \begin{subfigure}[b]{0.45\linewidth}
        \centering
        \includegraphics[width=\linewidth]{sh60k.jpg}
        \caption{SH-60K（シングルロータ）}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\linewidth}
        \centering
        \includegraphics[width=\linewidth]{CH_47J.jpg}
    ...
    \begin{subfigure}[b]{0.45\linewidth}
        ...
        \caption{Ka-32（二重反転式ロータ）}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\linewidth}
        \centering
        \includegraphics[width=\linewidth]{kmax.jpg}
        \caption{K-MAX（交差反転式ロータ）}
    \end{subfigure}

    \caption{ヘリコプタの様式\cite{ref1,ref2,ref3,ref4}}
    ...
    また,その振動荷重及びモーメントが胴体を加振することにより生ずるパイロット座席での加速度がパイロット人体の疲労や不快感を生起することとなる（１１－１６）
    ...
    更には,今後コンパウンドヘリコプタに代表される高速で飛行できる回転翼機の需要が高まっていくことが予想されるため（１７）,
    ...
    振動の伝達を小さくするよう考えられたものである.類似の例として The Improved Rotor Isolation System （IRIS）（２３）The hydraulic antiresonant isolator（２４）
